...
public class Rescuer_Profile extends BaseRescuerActivity {
    ...
    private void deleteUserDocument(String uid, String userType, Runnable onComplete) {
        ...
        userDocRef.get()
                .addOnSuccessListener(documentSnapshot -> {
                    if (documentSnapshot.exists()) {
                        Log.d(TAG, "📄 Rescuer document exists, deleting now...");
                        // Delete the document
                        userDocRef.delete()
                                .addOnSuccessListener(aVoid -> {
                                    Log.d(TAG, "✅ Rescuer document deleted successfully from Firestore");
                        ...
                        Log.d(TAG, "⚠️ Rescuer document does not exist, may have been already deleted");
                        onComplete.run();
                    }
                })
                .addOnFailureListener(e -> {
                    Log.e(TAG, "⚠️ Failed to check if rescuer document exists: " + e.getMessage());
                    // Try to delete anyway
                    userDocRef.delete()
                            .addOnSuccessListener(aVoid -> {
                                Log.d(TAG, "✅ Rescuer document deleted (existence check failed but delete succeeded)");
                                onComplete.run();
                            })
                            .addOnFailureListener(ex -> {
                                Log.e(TAG, "❌ Failed to delete rescuer document: " + ex.getMessage());
    ...
    private void retryDeleteDocument(String uid, String userType, int attempt, Runnable onComplete) {
        if (attempt >= 3) {
            Log.e(TAG, "❌ Failed to delete rescuer document after 3 attempts");
            onComplete.run();
            return;
        }
    ...
    private void verifyDocumentDeletion(String uid, String userType, int attempt, Runnable onComplete) {
        ...
        db.collection("Sagip")
                ...
                .addOnSuccessListener(documentSnapshot -> {
                    if (!documentSnapshot.exists()) {
                        Log.d(TAG, "✅ Verified: Rescuer document successfully deleted");
                        onComplete.run();
                    } else {
