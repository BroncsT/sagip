...
public class Senior_Profile extends BaseProfileActivity {
    ...
    private void deleteUserDocument(String uid, String userType, Runnable onComplete) {
        ...
        userDocRef.delete()
                .addOnSuccessListener(aVoid -> {
                    Log.d(TAG, "✅ User document delete() called successfully");
    ...
    private void verifyAndForceDeleteDocument(String uid, String userType, int attempt, Runnable onComplete) {
        ...
        userDocRef.get()
                .addOnSuccessListener(documentSnapshot -> {
                    if (!documentSnapshot.exists()) {
                        Log.d(TAG, "✅ VERIFIED: User document successfully deleted from Firestore");
                        onComplete.run();
                    } else {
    ...
    private void retryDeleteDocument(String uid, String userType, int attempt, Runnable onComplete) {
        if (attempt >= 5) {
            Log.e(TAG, "❌ Failed to delete user document after 5 retry attempts - using force delete");
            forceDeleteDocument(uid, userType, onComplete);
            return;
        }
